...
| Pure format conversion/export | Choose an appropriate conversion path: `convertx-file-converter`, local LibreOffice/soffice, or another available tool |

For requests that only convert an existing PPT/PPTX to another format, do not edit the deck. Use judgment based on the requested format, available tools, file size, and whether the conversion service exposes the needed target. ConvertX is available through the `convertx-file-converter` skill, but local LibreOffice/soffice is also acceptable when it is the simpler or better-supported path.
